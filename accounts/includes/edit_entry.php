...
if(isset($worker_id)){
?>
	<h2 align="center">Edit Your Resume<hr>
	<img src="../admin_area/worker_images/<?php if(!empty($image)) echo $image; else echo "noimg.jpg"; ?>" height="100" width="100"></h2>
    <form name="resume" onsubmit="return valid(this.form);" action="index.php?editres&edit_entry"  method="post" enctype="multipart/form-data" class="form-horizontal">
...
<?php 
} else{ ?> <p class="text-danger" style="font-size:45px">You've not uploaded your Resume yet.</p>
<?php }?>
</body>
</html>
<?php
	if(isset($_POST['update'])){
        ...
        move_uploaded_file($image_tmp,"../admin_area/worker_images/$new_image");
		if(!empty($new_image))
			$image = $new_image;

		$update_product = "update self set name = '$name', dob = '$dob', occ_id = '$occ_id', genre_id = '$genre_id',
		religion = '$religion', gender = '$gender', fee = '$fee', state = '$state', city = '$city',
		area = '$area', experience = '$experience', work_desc = '$work_desc', family_desc = '$family_desc',
		image = '$image' where member_id='".$_SESSION['user_id']."'";
		$update_pro = mysqli_query($con, $update_product);
		if($update_pro) {
			echo"<script>alert ('entry has been updated.')</script>";
            echo"<script>window.open('index.php?editres&edit_entry','_self')</script>";
		}
	}
?>